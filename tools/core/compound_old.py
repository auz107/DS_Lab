...
class compound(object):
    ...
    def __init__(self, id, compartment = None, name = None, synonyms = [], Kegg_id = None, ModelSeed_id = None, formula = None, molecular_weight = None, charge = None, reactions = [], reactant_reactions = [], product_reactions = [], concentration = None, deltaGf_range = None, biomass_yield = None, ms = None, notes = None, warnings = True): 

        # Warnings 
        if not isinstance(warnings,bool):
            raise ValueError('warnings must be True or False')
        else:
        ...
        if self.reactions == [] and reactant_reactions != [] and product_reactions != []:
            self.reactions = list(set(reactant_reactions + product_reactions))
        elif self.reactions == [] and reactant_reactions != [] and product_reactions == []:
            self.reactions = list(set(reactant_reactions))
        elif self.reactions == [] and reactant_reactions == [] and product_reactions != []:
            self.reactions = list(set(product_reactions))
        print self.id,'\treactions = ',self.reactions
        self.reactions = sorted(self.reactions,key=lambda x:x.id)

        # List of reaction objects in which this metabolite participates as a
        # reactant 
        self.reactant_reactions = reactant_reactions
        # The following statement is in the try block because self.reaciotns
        # may not be necessarily a list of reaciton objects. This is because sometimes
        # the user may just provide strings for the filed reactions, instead of
        # reaciton objects
        try:
    ...
    def biomass_yield_calc(self,fba_model = None, stdout_msgs = True, warnings = None):
        """
        ...
        if len(exch_rxn) == 0:
            ...
            if fba_model == None:
                if warnings:
                    print 'WARNING! No fba_model is provided, the current model.fba_model is used to compute the biomass yield'

                if self.model.fba_model != None:
                    self.model.fba(build_new_optModel = False,store_opt_fluxes = True, stdout_msgs = False)
                else: 
                    self.model.fba(build_new_optModel = True,store_opt_fluxes = True, stdout_msgs = False)
            ...
            if fba_solution['exit_flag'] == 'globallyOptimal':
                if exch_rxn.flux < 0:
                    ...
                    if  self.warnings:
                        print 'WARNING! The fba problem to compute the biomass yield for metabolite ' + self.id + ' resulted in a non-negative flux value for the corresponding exchange reaction. No value is assigned to biomass_yield'
            else:
                self.biomass_yield = None
                if  self.warnings:
                    print 'WARNING! The fba problem to compute the biomass yield for metabolite ' + self.id + ' was not solved to optimality. No value is assigned to biomass_yield'

            # Set the lower bound on exchange reaction back to what it was before
            exch_rxn.flux_bounds[0] = exch_rxn_LB

    def ms_calc(self,fba_model = None, stdout_msgs = True, warnings = None):
        """
        ...
        if len(exch_rxn) == 0:
            ...
            if fba_model == None:
                if warnings:
                    print 'WARNING! No fba_model is provided, the current model.fba_model is used to compute ms'
                ...
                if self.model.fba_model != None:
                    self.model.fba(build_new_optModel = False,store_opt_fluxes = False, stdout_msgs = False)
                else:
                    self.model.fba(build_new_optModel = True,store_opt_fluxes = False, stdout_msgs = False)
            ...
            self.model.fba_model.optModel.del_component('objectiveFunc')
            self.model.fba_model.optModel.objectiveFunc = pyomo.Objective(rule=self.model.fba_model.objectiveFunc_rule, sense = pyomo.maximize)
